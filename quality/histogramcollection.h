...
class HistogramCollection : public Serializable {
 public:
  ...
  explicit HistogramCollection(unsigned polarizationCount)
  ...
  void Add(const unsigned antenna1, const unsigned antenna2,
    ...
    for (size_t i = 0; i < sampleCount; ++i) {
      const double amplitude = sqrt(values[i].real() * values[i].real() +
                                    values[i].imag() * values[i].imag());
      totalHistogram.Add(amplitude);
      if (isRFI[i]) rfiHistogram.Add(amplitude);
    }
  }
  ...
  bool Empty() const {
    if (_polarizationCount == 0) return true;
    for (unsigned p = 0; p != _polarizationCount; ++p) {
  ...
  void Add(const unsigned antenna1, const unsigned antenna2,
           const unsigned polarization, Image2DCPtr image, Mask2DCPtr mask);

  void Add(const unsigned antenna1, const unsigned antenna2,
           const unsigned polarization, Image2DCPtr image, Mask2DCPtr flagMask,
