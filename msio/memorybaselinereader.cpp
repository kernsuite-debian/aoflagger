...
void MemoryBaselineReader::PerformReadRequests(ProgressListener& progress) {
  ...
  for (size_t i = 0; i != _readRequests.size(); ++i) {
    ...
    BaselineID id(request.antenna1, request.antenna2, request.spectralWindow,
                  request.sequenceId);
...
void MemoryBaselineReader::writeFlags() {
  ...
  msSelection.Process(
      [&](size_t rowIndex, size_t sequenceId, size_t timeIndexInSequence) {
        ...
        std::map<BaselineID, std::unique_ptr<Result>>::iterator resultIter =
...
bool MemoryBaselineReader::IsEnoughMemoryAvailable(
  ...
  if (size * 2 >= totalMem) {
    Logger::Warn << (size / 1000000) << " MB required, but "
                 << (totalMem / 1000000)
                 << " MB available.\n"
                    "Because this is not at least twice as much, direct read "
