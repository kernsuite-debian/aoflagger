...
void test() {
  ...
  vec = Vec(1000);
  vec.push_back(16);
  BOOST_CHECK(insert_uninitialized(vec, vec.begin() + 1001, 1) - vec.begin() ==
              1001);
  BOOST_CHECK(insert_uninitialized(vec, vec.begin() + 1001, 1) - vec.begin() ==
              1001);
  BOOST_CHECK(vec.size() == 1003);
...
class IdAllocater {
 public:
  ...
  IdAllocater<Tp> operator=(const IdAllocater<Tp>& rhs) {
    _id = rhs._id;
    return *this;
  }
...
void testAllocater() {
  ...
  vecC = std::move(vecA);
  BOOST_CHECK(vecA.end() - vecA.begin() >= 0);
...
BOOST_AUTO_TEST_CASE(all) {
  ...
  // std::cout << "is_pod<std::pair<int,int> > = " <<
  // std::is_pod<std::pair<int,int> >() << '\n'; std::cout <<
  // "is_trivial<std::pair<int,int> > = " << std::is_trivial<std::pair<int,int>
  // >() << '\n'; std::cout << "is_standard_layout<std::pair<int,int> > = " <<
  // std::is_standard_layout<std::pair<int,int> >() << '\n';
}
