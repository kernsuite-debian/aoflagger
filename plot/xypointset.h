...
class XYPointSet {
 public:
  ...
  void PushDataPoint(double x, double y) {
    if (std::isfinite(x)) _points.emplace_back(x, y);
  }
  ...
  double MaxX() const {
    if (_points.empty()) return std::numeric_limits<double>::quiet_NaN();
  ...
  void Sort() { std::sort(_points.begin(), _points.end()); }
  ...
  double XRangePositiveMin() const {
    if (_points.empty())
      ...
      std::vector<Point2D>::const_iterator iter =
          std::upper_bound(_points.begin(), _points.end(), Point2D(0.0, 0.0));
  ...
  bool HasTickLabels() const { return !_tickLabels.empty(); }
  const std::vector<std::string>& TickLabels() const { return _tickLabels; }
  void SetRotateUnits(bool rotateUnits) { _rotateUnits = rotateUnits; }
  bool RotateUnits() const { return _rotateUnits; }
  /**
   * Set the range that this point set minimally wants to have visualized. Other
   * point sets might request a larger range, which might enlarge this request.
   */
