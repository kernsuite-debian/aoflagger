...
class Mask2D : public boost::intrusive_ref_counter<Mask2D> {
 public:
  ...
  bool operator==(const Mask2D& rhs) const {
    if (_width != rhs._width || _height != rhs._height) return false;
    for (size_t y = 0; y < _height; ++y) {
      for (size_t x = 0; x < _width; ++x)
        if (_values[y][x] != rhs._values[y][x]) return false;
    }
    return true;
  }
   ...
   * @see Stride()
   */
  bool* ValuePtr(size_t x, size_t y) { return &_values[y][x]; }
   ...
   * @see Stride()
   */
  const bool* ValuePtr(size_t x, size_t y) const { return &_values[y][x]; }

  bool* Data() { return _valuesConsecutive; }

  const bool* Data() const { return _valuesConsecutive; }
  ...
  void SetAllVertically(size_t x) {
    for (size_t y = 0; y < _height; ++y) _values[y][x] = NewValue;
  }
  ...
  void Invert() {
    for (size_t y = 0; y < _height; ++y) {
      for (size_t x = 0; x < _width; ++x) _values[y][x] = !_values[y][x];
    }
  }
  ...
  Mask2D MakeXYFlipped() const {
    ...
    for (size_t y = 0; y < _height; ++y) {
      for (size_t x = 0; x < _width; ++x) mask._values[x][y] = _values[y][x];
    }
    return mask;
  }
  ...
  size_t GetCount() const {
    ...
    for (size_t y = 0; y < _height; ++y) {
      for (size_t x = 0; x < _width; ++x)
        if (BoolValue == _values[y][x]) ++count;
    }
    return count;
  }
