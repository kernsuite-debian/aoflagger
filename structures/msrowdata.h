 ...
 * If some of the meta data needs to be stored as well, use the MSRowDataExt
 * class.
 * @see MSRowDataExt
 */
class MSRowData : public Serializable {
 public:
  ...
  virtual void Unserialize(std::istream &stream) final override {
    ...
    for (size_t i = 0; i < size; ++i) _realData[i] = UnserializeFloat(stream);
    for (size_t i = 0; i < size; ++i) _imagData[i] = UnserializeFloat(stream);
  }
