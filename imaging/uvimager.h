...
class UVImager {
 public:
  ...
  /**
   * This function calculates the uv position, but it's not optimized for speed,
   * so it's not to be used in an imager.
   ...
   * @param [in] frequencyIndex the frequency index to calculate the u,v
   * position for
  ...
  static num_t GetFringeStopFrequency(size_t time, const Baseline& baseline,
                                      ...
                                      TimeFrequencyMetaDataCPtr metaData);
  // static double GetFringeCount(long double timeStart, long double timeEnd,
  // const Baseline &baseline, long double delayDirectionRA, long double
  // delayDirectionDec, long double frequency);
  static num_t GetFringeCount(size_t timeIndexStart, size_t timeIndexEnd,
                              unsigned channelIndex,
                              TimeFrequencyMetaDataCPtr metaData);

  static numl_t GetWPosition(numl_t delayDirectionDec, numl_t delayDirectionRA,
                             numl_t frequency, numl_t earthLattitudeAngle,
                             numl_t dx, numl_t dy) {
    ...
    num_t wPosition = (dx * raCosEnd - dy * raSinEnd) * (-decCos) / wavelength;
    return wPosition;
  }

  static numl_t TimeToEarthLattitude(unsigned x,
                                     TimeFrequencyMetaDataCPtr metaData) {
  ...
  num_t UVScaling() const { return _uvScaling; }
  ...
  static numl_t UVTrackLength(TimeFrequencyMetaDataCPtr metaData,
    ...
    while ((i + 1) != uvw.end()) {
      ...
      ++n;
      const numl_t du = n->u - i->u, dv = n->v - i->v;
      length += sqrtnl(du * du + dv * dv);
      i = n;
    }
    return length * frequencyHz / SpeedOfLight();
  }
  numl_t ImageDistanceToFringeSpeedInSamples(
      numl_t imageDistance, double frequencyHz,
      TimeFrequencyMetaDataCPtr metaData) const {
    return ImageDistanceToDecRaDistance(imageDistance) *
           AverageUVDistance(metaData, frequencyHz) /
           (0.5 * (numl_t)metaData->UVW().size());
  }
