...
class SumThresholdTest : public UnitTest {
...
#endif
#ifdef __AVX2__
			AddTest(SimpleVerticalSumThresholdAVX<false>(), "Simple SumThreshold AVX case (vertical)");
			AddTest(VerticalSumThresholdAVX<false>(), "SumThreshold optimized AVX version (vertical)");
			AddTest(StabilityAVX<false>(), "SumThreshold stability (AVX)");
			
			AddTest(SimpleVerticalSumThresholdAVX<true>(), "Simple SumThreshold AVX Dumas case (vertical)");
			AddTest(HorizontalSumThresholdAVXDumas(), "SumThreshold optimized AVX Dumas version (horizontal)");
			AddTest(VerticalSumThresholdAVX<true>(), "SumThreshold optimized AVX Dumas version (vertical)");
			AddTest(StabilityAVX<true>(), "SumThreshold stability (AVX Dumas)");
#endif
		}
	...
	private:
		...
		struct HorizontalSumThresholdAVXDumas : public Asserter
		{
			void operator()();
		};
		template<bool dumas>
...
#ifdef __AVX2__
void SumThresholdTest::HorizontalSumThresholdAVXDumas::operator()()
{
	const unsigned
		width = 2048,
		height = 256;
	...
	for(unsigned i=0;i<9;++i)
	{
		...
		std::stringstream s;
		s << "Equal AVX Dumas and reference masks produced by SumThreshold length " << length << ", threshold " << threshold;
		MaskAsserter::AssertEqualMasks(mask2, mask1, s.str());
	}
}

template<bool dumas>
void SumThresholdTest::VerticalSumThresholdAVX<dumas>::operator()()
{
	const unsigned
		width = 2048,
		height = 256;
	...
	for(unsigned i=0;i<9;++i)
	{
		...
		if(dumas)
			SumThreshold::VerticalAVXDumas(image.get(), &mask2, &vScratch, length, threshold);
		else
			SumThreshold::VerticalLargeAVX(image.get(), &mask2, &scratch, length, threshold);
		
		if(length != 32 || dumas) { // I think there was a numerical issue on some platforms with 32, so just skip.
		  std::stringstream s;
		  s << "Equal AVX Dumas and reference masks produced by SumThreshold length " << length;
		  MaskAsserter::AssertEqualMasks(mask2, mask1, s.str());
		}
	}
}
...
template<bool dumas>
void SumThresholdTest::SimpleVerticalSumThresholdAVX<dumas>::operator()()
{
	const unsigned
		width = 8,
		height = 8;
...
template<bool dumas>
void SumThresholdTest::StabilityAVX<dumas>::operator()()
{
	Mask2D
		maskA = Mask2D::MakeSetMask<false>(1, 1),
		maskB = Mask2D::MakeSetMask<false>(2, 2),
