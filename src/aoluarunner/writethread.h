...
{
public:
	WriteThread(rfiStrategy::ImageSet& imageSet, size_t calcThreadCount, std::mutex* ioMutex);
	~WriteThread();
	
	void SaveFlags(const TimeFrequencyData& data, rfiStrategy::ImageSetIndex& imageSetIndex);
...
private:
	...
	struct BufferItem {
		BufferItem(const std::vector<Mask2DCPtr> &masks, const rfiStrategy::ImageSetIndex &index)
		...
		std::unique_ptr<rfiStrategy::ImageSetIndex> _index;
	};
