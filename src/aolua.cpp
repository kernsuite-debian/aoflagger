...
void generalInfo()
{
	Logger::Info << 
		...
		"This program will execute a Lua flagging script that can be created with the RFI gui\n"
...
int main(int argc, char **argv)
{
	if(argc == 1)
	{
		...
		"     Run the strategy on the given baseline types. The default is to run the strategy on\n"
		"     all cross-correlation baselines. This parameter has no effect for single-dish observations.\n"
		"  -combine-spws\n"
		"     Join all SPWs together in frequency direction before flagging\n"
		"  -preamble <statement>\n"
		"     Runs the specified Lua statement before starting to flag. This is typically used to\n"
		"     define a variable, e.g. -preamble \"bandpassfile = mybandpass.txt\".\n"
		"\n"
	...
	while(parameterIndex < (size_t) argc && argv[parameterIndex][0]=='-')
	{
		...
		{
			parameterIndex++;
			options.strategyFile = argv[parameterIndex];
		}
		...
		{
			options.interval = std::pair<size_t,size_t>(
		...
		{
			++parameterIndex;
			options.maxIntervalSize = atoi(argv[parameterIndex]);
		}
		else if(flag == "baselines")
		{
			...
			if(bTypes == "all")
				options.baselineSelection = rfiStrategy::All;
			else if(bTypes == "cross")
				options.baselineSelection = rfiStrategy::CrossCorrelations;
			else if(bTypes == "auto")
				options.baselineSelection = rfiStrategy::AutoCorrelations;
			else {
	...
	try {
		...
		if(!options.threadCount)
			options.threadCount = System::ProcessorCount();
		Logger::Debug << "Number of threads: " << options.threadCount.get() << "\n";
