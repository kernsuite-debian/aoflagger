...
TwoDimensionalPlotPage::TwoDimensionalPlotPage(
    AOQPlotPageController* controller)
...
std::set<AOQPlotPageController::SelectedPol>
TwoDimensionalPlotPage::GetSelectedPolarizations() const {
  std::set<AOQPlotPageController::SelectedPol> pols;
  if (_polPPButton.get_active()) pols.insert(AOQPlotPageController::PolPP);
  if (_polPQButton.get_active()) pols.insert(AOQPlotPageController::PolPQ);
  if (_polQPButton.get_active()) pols.insert(AOQPlotPageController::PolQP);
  if (_polQQButton.get_active()) pols.insert(AOQPlotPageController::PolQQ);
  if (_polIButton.get_active()) pols.insert(AOQPlotPageController::PolI);
  return pols;
}

std::set<AOQPlotPageController::PhaseType>
TwoDimensionalPlotPage::GetSelectedPhases() const {
...
void TwoDimensionalPlotPage::initStatisticKindButtons(Gtk::Toolbar& toolbar) {
  ...
  _countButton.signal_clicked().connect(
      sigc::mem_fun(*_controller, &AOQPlotPageController::UpdatePlot));
  ...
  _varianceButton.signal_clicked().connect(
      sigc::mem_fun(*_controller, &AOQPlotPageController::UpdatePlot));
  toolbar.append(_varianceButton);

  //_dCountButton.signal_clicked().connect(sigc::mem_fun(*_controller,
  //&AOQPlotPageController::UpdatePlot)); toolbar.append(_dCountButton);
...
void TwoDimensionalPlotPage::initPlotButtons(Gtk::Toolbar& toolbar) {
  ...
  _logarithmicButton.signal_clicked().connect(
      sigc::mem_fun(*this, &TwoDimensionalPlotPage::onLogarithmicClicked));
  toolbar.append(_logarithmicButton);

  _zeroAxisButton.signal_clicked().connect(
      sigc::mem_fun(*this, &TwoDimensionalPlotPage::updatePlotConfig));
